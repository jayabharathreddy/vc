var reportLodingLabel = '<div class="control-group" align="center"><label class="control-label">Processing............!</label><div class="controls"><progress id="loadingbar" value="70"max="100" /></div></div>';
	...
	$("#submitAdvance").click(function(){	
		...
		var advanceReportType= $("#advanceType").val();
		 ...
		 if(advanceReportType=='')
		 {
			 alert('Select a Advance option');
		 }
		 else
		 {
			    $("#advanceTaskLoading").empty();
				$("#advanceLoading").empty();
				$("#advanceTaskLoading").append(reportLodingLabel);
				$("#advanceLoading").append(reportLodingLabel);
			 ...
			 if(advanceReportType=='Today')
				...
				{			
				  loadRestForSingleDay(urlForServer + '/report/SingleDayReport'+"/"+day+"/"+month+"/"+year,'Advance');
				}
				else if(reportType =="monthly")
				{
					 loadRestForSingleDay(urlForServer + '/report/MonthlyDayReport'+"/"+day+"/"+month+"/"+year+"/"+Duration,'Advance');
				}
	...
	$("#submit").click(function(){	
		...
		else
		{
			...
			$("#taskLoading").empty();
			$("#loading").empty();
			$("#taskLoading").append(reportLodingLabel);
			$("#loading").append(reportLodingLabel);
			...
			loadRestForSingleDay(urlForServer + '/report/SingleDayReport'+"/"+day+"/"+month+"/"+year,'Basic');
		}
	 ...
	 $("#generateReportBtn").click(function(){ 
		 ...
		 var restfulUrl=urlForServer+"report/get/" ;
				...
				if(celValue!=null)
				{
                  ...
                  loadRestfuldata(restfulPageUrl,'Basic');
				}
				else
				{
	 ...
	 $("#advanceGenerateReportBtn").click(function(){ 
		 var myGrid = $('#advanceTaskTable'),
		     selRowId = myGrid.jqGrid ('getGridParam', 'selrow'),
		     celValue = myGrid.jqGrid ('getCell', selRowId, 'taskId');
		 ...
		 var restfulUrl=urlForServer+"report/get/" ;
				...
				if(celValue!=null)
				{
				  $("#advanceLoading").removeClass("hidden");
				  var restfulPageUrl = restfulUrl + celValue ;//+ 'page'; http://192.168.1.65:8080/api.vc/v1.0/CallList
                  loadRestfuldata(restfulPageUrl,'Advance');
				}
				else
				{
					alert('Select any one TaskSid');
	...
	function loadRestForSingleDay(pageUrl ,reportType) {
		...
		{
			$("#advanceTaskLoading").removeClass("hidden");
		}
  		...
  		$.ajax({
  			...
  			success:function(data){
  				...
  				if(data1 ==null)
  				{
  					...
  					{
  						//Advance
  						 $("#advanceTaskLoading").addClass("hidden");
  						 $("#submitadvance").removeAttr("disabled");
  						 $("#advanceType").removeAttr("disabled");
  				...
  				{	
  					...
  					else
  					{
  						//Advance
  						 $("#advanceTaskLoading").addClass("hidden");
  						 $("#advanceTaskHeading").removeClass("hidden");  						 
  						 $("#submitadvance").attr("disabled", true);
  	  					 $("#advanceType").attr("disabled", true);
  	  				     $("#advanceTaskTable ").jqGrid("GridUnload");
  	  				    
  	  				    label1 = '#advanceTaskTable';
  	  				    label2 = '#advanceTablePage';
	...
	function loadRestfuldata(pageUrl,reportType) {
    		...
    		$.ajax({
    			...
    			success:function(data){
    				...
    				var list2,list3,list4,list5,list6,list7,list8;
    				...
    				else
    				{
    				 list2 ="#advanceList2";
   					 ...
   					 list8 ="#advanceList8";
   					    ...
   					    if(data[0].taskSid!='null' && data[0].taskSid!='')
   	    	        	$("#advanceReport").removeClass("hidden");
    	        ...
    	        /*	$("#list1").jqGrid({
    	        		...
    	        		width: 500,
    	        		 colNames:['Task Sid','Broadcast Sid','Start Day','Start Month','Start Year','Start Hour','Start Minute','End Day','End Month','End Year','End Hour','End Minute'],
    	        	   	colModel:[
    	        	   	    {name:'taskSid',index:'taskSid', width:30},
    	        	   		{name:'broadcastSid',index:'broadcastSid', width:50},
    	        	   		{name:'startDay' , index:'startDay' , width:50, align:"center"},
    	        	   		{name:'startMonth', index:'startMonth' , width:50 ,align:"center"},
    	        	   		{name:'startYear' , index:'startYear', width:50 ,align:"center"},
    	        	   		{name:'startHour', index:'startHour', width:40 ,align:"center"},
    	        	   		{name:'startMinute', index:'startMinute', width:40 ,align:"center"},
    	        	   		{name:'endDay' , index:'endDay', width:40 ,align:"center"},
    	        	   		{name:'endMonth', index:'endMonth', width:40 ,align:"center"},
    	        	   		{name:'endYear', index:'endYear', width:40 ,align:"center"},
    	        	   		{name:'endHour', index:'endHour', width:40 ,align:"center"},
    	        	   		{name:'endMinute', index:'endMinute', width:40 ,align:"center"}
    	        	   ],
    	        	   	caption: "Task Details"
    	        	 });
    	        	...
    	        	$(list2).jqGrid({
    	        		...
    	        		width: 650,
    	        		 colNames:['Task Sid','Total Scheduled','Total Attempts','Total Remaining','Total Time','Total Charge'],
    	        	   	colModel:[
    	        	   	    {name:'taskSid',index:'taskSid', width:80},
    	        	   		{name:'totalScheduled',index:'totalScheduled', width:110 ,align:"center"},
    	        	   		{name:'totalAttempts',index:'totalAttempts', width:110 ,align:"center"},
    	        	   		{name:'totalRemaining',index:'totalRemaining', width:110 ,align:"center"},
    	        	   		{name:'totalMinutesSeconds',index:'totalMinutesSeconds', width:110 ,align:"center"},
    	        	   		{name:'totalCharge',index:'totalCharge', width:110 ,align:"center"}
    	        	   ],
    	        	...
    	        	$(list3).jqGrid({
    	        		...
    	        		width: 650,
    	        		 colNames:['Task Sid','Answered','Machine','Busy','Tty','Fax'],
    	        	   	colModel:[
    	        	   	    {name:'taskSid',index:'taskSid', width:80},
    	        	   		{name:'resultAnswered',index:'resultAnswered', width:110 ,align:"center"},
    	        	   		{name:'resultMachine',index:'resultMachine', width:110 ,align:"center"},
    	        	   		{name:'resultBusy',index:'resultBusy', width:110 ,align:"center"},
    	        	   		{name:'resultTty',index:'resultTty', width:110 ,align:"center"},
    	        	   		{name:'resultFax',index:'resultFax', width:110 ,align:"center"}
    	        	   ],
    	        	...
    	        	$(list4).jqGrid({
    	        		...
    	        		width: 650,
    	        		 colNames:['Task Sid','Invalid','DNC','Timed out','Cancelled'],
    	        	   	colModel:[
    	        	   	    {name:'taskSid',index:'taskSid', width:80},
    	        	   		{name:'resultInvalid',index:'resultInvalid', width:120 ,align:"center"},
    	        	   		{name:'resultDoNotCall',index:'resultDoNotCall', width:120 ,align:"center"},
    	        	   		{name:'resultTimedOut',index:'resultTimedOut', width:120 ,align:"center"},
    	        	   		{name:'resultCancelled',index:'resultCancelled', width:120 ,align:"center"}
    	        	...
    	        	$(list6).jqGrid({
    	        		...
    	        		width: 650,
    	        		 colNames:['Task Sid','Linkback Answered','Linkback Talkoff','Dropped At Transfer','Dropped At Hold','Dropped Total'],
    	        	   	colModel:[
    	        	   	    {name:'taskSid',index:'taskSid', width:80},
    	        	   		{name:'linkbacksAnswered',index:'linkbacksAnswered', width:130 ,align:"center"},
    	        	   		{name:'linkbackTalkOffs',index:'linkbackTalkOffs', width:110 ,align:"center"},
    	        	   		{name:'linkbacksDroppedAtTransfer',index:'linkbacksDroppedAtTransfer', width:130 ,align:"center"},
    	        	   		{name:'linkbacksDroppedAtHold',index:'linkbacksDroppedAtHold', width:120 ,align:"center"},
    	        	   		{name:'linkbacksDroppedTotal',index:'linkbacksDroppedTotal', width:110 ,align:"center"}
    	        	...
    	        	$(list7).jqGrid({
    	        		...
    	        		width: 650,
    	        		 colNames:['Task Sid','Average Message','Average Transfer','Onekey Connect','Talk Off','Average Totaltime','Average Cost'],
    	        	   	colModel:[
    	        	   	    {name:'taskSid',index:'taskSid', width:70},
    	        	   		{name:'linkbacksAvgMessage',index:'linkbacksAvgMessage', width:110 ,align:"center"},
    	        	   		{name:'linkbacksAvgTransfer',index:'linkbacksAvgTransfer', width:110 ,align:"center"},
    	        	   		{name:'linkbacksAvgOneKeyConnect',index:'linkbacksAvgOneKeyConnect', width:100 ,align:"center"},
    	        	   		{name:'linkbacksAvgTalkOff',index:'linkbacksAvgTalkOff', width:100 ,align:"center"},
    	        	   		{name:'linkbacksAvgTotalTime',index:'linkbacksAvgTotalTime', width:110 ,align:"center"},
    	        	   		{name:'linkbacksAvgCost',index:'linkbacksAvgCost', width:90 ,align:"center"}
    	        	   ],
    	        	...
    	        	$(list8).jqGrid({
    	        		...
    	        		width: 650,
    	        		 colNames:['Task Sid','Email Sent','Email Invalid','Email DNC'],
    	        	   	colModel:[
    	        	   	    {name:'taskSid',index:'taskSid', width:100},
    	        	   		{name:'emailsSent',index:'emailsSent', width:150 ,align:"center"},
    	        	   		{name:'emailsInvalid',index:'emailsInvalid', width:150 ,align:"center"},
    	        	   		{name:'emailsDNC',index:'emailsDNC', width:150 ,align:"center"}
    	        	...
    	        	{
    	        		$("#advanceLoading").addClass("hidden");
    	        	}
