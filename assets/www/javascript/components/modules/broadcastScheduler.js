...
$(document).ready(function() {
	...
	// $("#audioSrc").attr( 'src', audioLocation());
		$('#pickScheduleAnch').click(function() 
			{
				var componentId='pickScheduleAnch';
				getBroadcastGroupDesc(componentId);//ajax function
			});
		$('#pickMsgAnch').click(function() 
				{
					var componentId='pickMsgAnch';
					getAdminMessages(componentId); //ajax function
				});
...
function getBroadcastGroupDesc(componentId) 
{
	// Author: @sbashid
	// to fetch the admin broadcast details from mongo by hitting the vcApi

	// var url = urlForServer+"broadcast/user/"+gcLoginUserId; // fetch ordinary
	// broadcast(not an admin broadcast) details
	...
	$
			.ajax( {
				...
				success : function(responseText) {
					console
							.info("<-------Broadcast description returned with response as -------> "
									+ responseText);
					if (componentId == 'pickScheduleAnch') {
						pickSchedule(responseText);
					}

				},
				error : function() {
					console
							.error("<-------Error returned while fetching the broadcast description -------> ");

				}
			});
}

function pickSchedule(responseText) 
{
	//Author: @sbashid
	//to form the pick Schedule dropdown
	...
	for ( var i = 0; i < data.length; i++) {
		var obj = data[i];
		var displayText = obj['bgDescription'];
		if (displayText != null && displayText != '')
			$("#pickScheduleUl")
					.append(
							'<li><a tabindex="-1" href="#" >' + displayText + '</a></li>');
	}
	$("#pickScheduleAnch").attr('data-toggle', 'dropdown');
}

function pickMessage(responseText) 
{
	//Author: @sbashid
	//to form the pick Schedule dropdown
	...
	for ( var i = 0; i < data.length; i++) {
		var obj = data[i];
		var displayText = obj['fileName'];
		if (displayText != null && displayText != '')
			$("#pickMessageUl")
					.append(
							'<li><a tabindex="-1" href="#" >' + displayText + '</a></li>');
	}
	$("#pickMsgAnch").attr('data-toggle', 'dropdown');
}
